...
#pragma warning restore IDE0025
    ...
    public override bool isPlaying => webCamTexture != null && webCamTexture.isPlaying;
        ...
        private IEnumerator Initialize()
        {
            ...
            {
                // 1. Set a fallback just in case
                webCamDevice = availableSources[0];

                // 2. FORCE THE AI TO FIND THE FRONT CAMERA!
                for (int i = 0; i < availableSources.Length; i++)
                {
                    if (availableSources[i].isFrontFacing)
                    {
                        webCamDevice = availableSources[i];
                        Debug.Log($"<color=green>Front camera found and selected: {availableSources[i].name}</color>");
                        break; // Stop looking once we find it!
                    }
                }
            }
            Debug.Log("Selected camera: " + webCamDevice?.name);
            Debug.Log("Total devices: " + WebCamTexture.devices.Length);
        }
        ...
        if (_IsPermitted)
        {
          yield break;
                ...
                if (!Permission.HasUserAuthorizedPermission(Permission.Camera))
                {
                    ...
                    // Wait until user responds (accept OR deny)
                    while (!Permission.HasUserAuthorizedPermission(Permission.Camera) &&
                           !Permission.ShouldShowRequestPermissionRationale(Permission.Camera))
                    {
                        yield return null;
                    }
                }
